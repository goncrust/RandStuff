...
GLuint compileShaders() {
    ...
    const char *vertex_shader =
        readSourceFile("resources/shaders/uniforms/uniformsgetloc.vs");
...
int main() {
    ...
    GLuint loc_Pos = glGetAttribLocation(program, "Pos");
    if (loc_Pos == -1) {
        std::cout << "Error getting attrib location of 'Pos'" << std::endl;
        exit(1);
    }

    GLuint loc_ScaleX = glGetUniformLocation(program, "ScaleX");
    if (loc_ScaleX == -1) {
        std::cout << "Error getting uniform location of 'ScaleX'" << std::endl;
        exit(1);
    }

    GLuint loc_ScaleY = glGetUniformLocation(program, "ScaleY");
    if (loc_ScaleY == -1) {
        std::cout << "Error getting uniform location of 'ScaleY'" << std::endl;
        exit(1);
    }

    std::cout << "loc_Pos: " << loc_Pos << ", loc_ScaleX: " << loc_ScaleX
              << ", loc_ScaleY: " << loc_ScaleY << std::endl;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUniform1f(loc_ScaleX, 0.75);
        glUniform1f(loc_ScaleY, 0.35);
